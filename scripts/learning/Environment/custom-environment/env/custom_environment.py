...
class CustomEnvironment(ParallelEnv):
    ...
    def step(self, actions):
        ...
        if prisoner_action == 0 and self.prisoner_x > 0:
            self.prisoner_x -= 1
        if prisoner_action == 1 and self.prisoner_x < 6:
            self.prisoner_x += 1
        if prisoner_action == 2 and self.prisoner_y > 0:
            self.prisoner_y -= 1
        if prisoner_action == 3 and self.prisoner_y < 6:
            self.prisoner_y += 1    

        if guard_action == 0 and self.guard_x > 0:
            self.guard_x -= 1
        if guard_action == 1 and self.guard_x < 6:
            self.guard_x += 1
        if guard_action == 2 and self.guard_y > 0:
            self.guard_y -= 1
        if guard_action == 3 and self.guard_y < 6:
            self.guard_y += 1    
